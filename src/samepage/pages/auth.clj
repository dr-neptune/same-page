...
(defn register-user-page
  ...
  (layout/page-layout
   ...
   [:div {:class "max-w-md mx-auto bg-[#2a2136] p-6 rounded shadow-md"}
    ...
    (when error-message
      [:div {:class "bg-red-600 text-white p-2 rounded mb-4"} error-message])
    [:form {:action "/register" :method "post"
            :class "space-y-4"}
     ;; USERNAME
     [:div
      [:label {:class "block font-semibold"} "Username (all lowercase, no spaces):"]
      [:input {:type "text"
               :name "name"
     ...
     [:div
      [:label {:class "block font-semibold"} "Display Name (optional):"]
      [:input {:type "text"
               :name "display_name"
               :class "w-full p-2 border border-gray-300 rounded bg-[#2f2b3b] text-[#e0def2]"}]]
